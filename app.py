...
# Link direto para a aba 'BASE' (gid=0) conforme sua planilha
...
# --- FUNÇÃO DE CLIMA (RESTAURADA E CORRIGIDA PARA CELSIUS) ---
def pegar_clima():
    try:
        # &m força o sistema métrico (Celsius)
...
def carregar_base():
    try:
        ...
        m_row, m_col = -1, -1
        # BUSCA REFORÇADA: Procura o 'MODELO' que tem dados reais abaixo dele (Coluna G)
        ...
        for i in range(len(dados)):
            ...
            unidade = pd.to_numeric(dados.iloc[i, m_col+1].replace(',', '.'), errors='coerce')
            descricao = str(dados.iloc[i, m_col+2]).strip()
            
            # Localiza a UPS (Célula)
            for offset in [3, 4, 5]:
...
def calcular(df_in, df_ba, h_ini, n_dia, tem_gin, regra_destino):
    ...
    def aplicar_conversao(row):
        ...
        orig = row['CEL_ORIGEM']
        n_orig = REGRAS_HORARIOS.get(orig, {"n_nat": 5})['n_nat']
...
if not base.empty:
    ...
    col_tit.header(f"📋 Planejamento: {sel_ups}")
