...
def pegar_clima():
    try:
        # Adicionado &m para forçar sistema métrico (Celsius)
...
def carregar_base():
    try:
        ...
        # Busca dinâmica: encontra a coluna onde está escrito "MODELO"
...
try:
    ...
    if not base.empty:
        ...
        if st.button("🚀 Gerar Planejamento"):
            if not df_editor.empty and df_editor["Equipamento"].any():
    ...
    else: st.error("⚠️ Verifique a planilha ou o nome das colunas.")
